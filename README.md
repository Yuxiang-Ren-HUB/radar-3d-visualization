...
download_kokx_case.py                下载KOKX Nor'easter个例雷达数据(AWS unidata-nexrad-level2桶)
radar_hrrr_blend_kokx.py              KOKX个例：自建简单融合方案
radar_hrrr_steps_blend_kokx.py        KOKX个例：STEPS集合融合方案
kokx_quicklook.png                     KOKX个例低层反射率快照(核查降水范围用)
radar_hrrr_blend_kokx_CSI_comparison.png      KOKX：三方法CSI对比
radar_hrrr_blend_kokx_snapshots.png           KOKX：对应快照对比
radar_hrrr_stepsblend_kokx_CSI_4way.png       KOKX：四方法CSI对比
radar_hrrr_stepsblend_kokx_snapshots_5way.png KOKX：五方法快照对比
radar_hrrr_stepsblend_kokx_ensemble_spread.png    KOKX：STEPS集合成员离散度
radar_hrrr_stepsblend_kokx_hrrr_interp_check.png  KOKX：HRRR插值核查
```

`.gitignore` 排除了 `plotly_fragment*.html`（构建过程的中间产物，内容已完整包含在对应的 `*_final.html` 里）、
`_anim_volume_cache.npz`、`_radar_hrrr_blend*_cache.npz`（网格化中间结果缓存）和
`radar_hrrr_stepsblend*_ensemble*.npy`（STEPS集合原始输出，单个种子就有~125MB，可由脚本重新生成，不必版本化）。
...
- 集合成员数(3或8)、噪声/速度扰动方法等参数仍是接近默认值，没有针对个例专门调优。

## 第二个例：KOKX Nor'easter —— 检验结论是否个例特定

Rolling Fork是快速移动、中途从孤立超级单体重组的强对流系统。为了检验"STEPS集合融合总是更好"
这个结论是不是个例特定的，换了一个性质完全相反的个例重跑一遍：**2023-03-13/14 东北沿岸低压
(Nor'easter)**，雷达站KOKX(纽约Upton)，慢速移动、大范围层状云降水、结构稳定（低层反射率快照
显示降水覆盖了雷达探测范围约80%的面积，见`kokx_quicklook.png`）。数据源同样是HRRR
(2023-03-14 00Z起报) + AWS历史雷达存档(这次走的是`unidata-nexrad-level2`镜像桶，
`noaa-nexrad-level2`官方桶对匿名HeadObject/ListObjects请求拒绝访问，`GetObject`本身倒是允许，
细节见`download_kokx_case.py`)。

**结果：结论确实是个例特定的，不能一概而论。**

| 时效 | 纯雷达 | 纯HRRR | 简单融合 | STEPS融合 |
|---|---|---|---|---|
| +1h | 0.250 | 0.148 | 0.228 | **0.026**（大幅落后）|
| +2h | 0.179 | 0.145 | 0.148 | 0.109（仍落后）|
| +3h | 0.140 | 0.157 | 0.126 | **0.192**（反超）|
| +4h | 0.097 | 0.121 | 0.099 | **0.134**（领先）|
| +5h | 0.094 | 0.072 | 0.072 | 0.079（小幅领先）|
| +6h | 0.031 | 0.039 | 0.039 | **0.061**（领先）|

（30dBZ阈值CSI，20/40dBZ阈值同一趋势）。跟Rolling Fork（STEPS在全部6个时效点都赢）完全不同：
这里雷达外推本身在+1h就已经很强（因为系统移动慢，覆盖率到+6h还有41%，不像Rolling Fork第5小时
分析域就100%被"掏空"），而STEPS集合均值在+1h反而**明显更差**——五方法快照对比图
(`radar_hrrr_stepsblend_kokx_snapshots_5way.png`)能看到，STEPS这一行在+1h把实况里清晰的
30-40dBZ强回波核抹平成了一片橙黄色。这是**集合预报常见的"集合均值平滑"陷阱**：集合成员之间在
短时效就已经出现了随机噪声导致的空间错位，直接对均值场算CSI这种精确阈值指标，天然会因为强信号
被平均抵消而吃亏（更合适的做法是用CRPS等概率检验指标，或者对每个成员单独算CSI再平均，而不是先
平均场再算）。到+3h以后，雷达自身开始衰减、HRRR权重上升，STEPS的尺度分解优势才体现出来、重新反超。

**结论**：STEPS集合融合的优势主要来自"雷达技巧崩溃得多快"——崩溃越快（Rolling Fork），STEPS的
尺度分解+自相关建模就越有价值；雷达本身撑得住的场景（KOKX这种慢速层状云降水），STEPS在最短时效
反而可能因为集合均值平滑而"帮倒忙"，只有中长时效才体现优势。
