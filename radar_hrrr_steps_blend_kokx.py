...
复用 radar_hrrr_blend.py 已经缓存好的雷达/HRRR网格化数据 (_radar_hrrr_blend_kokx_cache.npz)，
避免重新下载/网格化。
"""
import os
import numpy as np
...
OUT_PREFIX = "radar_hrrr_stepsblend_kokx"
...
fig.suptitle("雷达 / HRRR / 简单融合 / STEPS集合融合 —— 四方法CSI对比 (KOKX Nor'easter个例)")
...
fig2.suptitle("五方法快照对比 —— 实况/雷达外推/HRRR/简单融合/STEPS集合融合 (KOKX Nor'easter个例)")
